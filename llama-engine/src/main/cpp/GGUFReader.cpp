// Adaptado de OfflineLLM (jegly/OfflineLLM, Apache-2.0) para
// com.termux.llm.GGUFReader. Lee metadata de un .gguf (context_length,
// chat_template) sin cargar el modelo completo.
...
Java_com_termux_llm_GGUFReader_getGGUFContextNativeHandle(JNIEnv* env, jobject thiz, jstring modelPath) {
...
extern "C" JNIEXPORT jlong JNICALL
Java_com_termux_llm_GGUFReader_getContextSize(JNIEnv* env, jobject thiz, jlong nativeHandle) {
...
Java_com_termux_llm_GGUFReader_getChatTemplate(JNIEnv* env, jobject thiz, jlong nativeHandle) {
    gguf_context* ggufContext       = reinterpret_cast<gguf_context*>(nativeHandle);
...
// No presente en el smollm.cpp original de OfflineLLM (leak real: el
// gguf_context nunca se liberaba) — agregado acá para que GGUFReader.kt
// pueda liberar la memoria nativa explícitamente tras leer la metadata.
extern "C" JNIEXPORT void JNICALL
Java_com_termux_llm_GGUFReader_freeGGUFContext(JNIEnv* env, jobject thiz, jlong nativeHandle) {
    gguf_context* ggufContext = reinterpret_cast<gguf_context*>(nativeHandle);
