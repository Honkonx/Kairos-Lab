...
Fuente: VictorH028/no-root-logger (https://github.com/VictorH028/no-root-logger),
portado para la instrumentación "sin root" de compil-apk-termux (ver
modulos/apk.sh, cmd_instrument) — usado tal cual salvo 2 cambios: la clase de
...
Uso:
    ...
    python3 smali_hook.py archivo.smali --list --json
"""
...
REMOTE_LOGGER_CLASS = "Lcom/kairos/logger/RemoteLogger;"
EXTRA_REGISTERS = 4
...
class HookInjector:
    ...
    def load(self):
        # Bug real del proyecto original (VictorH028/no-root-logger): esta función exigía
        # SIEMPRE un --method que matcheara, aunque el propio README documenta "--list" sin
...
Ejemplos:
  ...
  python3 smali_hook.py archivo.smali --method Sf --action both --auto-registers
    ...
    parser.add_argument('--json', action='store_true', help='Con --list: salida JSON sin colores (para parsear)')
    ...
    if not (args.list and args.json):
