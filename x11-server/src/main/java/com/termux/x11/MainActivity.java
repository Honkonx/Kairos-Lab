...
//        preferences.registerOnSharedPreferenceChangeListener((sharedPreferences, key) -> onPreferencesChanged(key));
        ...
        findViewById(R.id.preferences_button).setOnClickListener((l) -> {
            // Kairos: en el embed no existe la TermuxActivity host de linbox (que era quien
            // seteaba termuxActivityListener), así que se abre LoriePreferences como actividad
            // standalone — patrón de termux-x11 oficial
            // (lorie/src/main/java/com/termux/x11/MainActivity.java:177). Los cambios aplican
            // en vivo porque LoriePreferenceFragment reemite ACTION_PREFERENCES_CHANGED y este
            // Activity lo recibe (receiver, ver abajo) → onPreferencesChanged(). Ver
            // docs/x11/X11_EMBEBIDO.md.
            startActivity(new Intent(this, LoriePreferences.class) {{
                setAction(Intent.ACTION_MAIN);
            }});
        });
        // Kairos: botón flotante transparente (ver main_activity.xml) — siempre visible
        // encima de la sesión activa, a diferencia de preferences_button (solo en el stub
        // de no-conectado). Mismo diálogo que showExitDialog()/onBackPressed().
        findViewById(R.id.x11_float_menu_button).setOnClickListener((l) -> showExitDialog());
...
//                        toggleKeyboardVisibility(MainActivity.this);
        ...
        // el visor completamente sordo al tacto salvo hover (stylus/mouse sin tocar).
        // Restaurado el dispatch real, igual al termux-x11 oficial (ver
        // referencia/termux/termux-x11-master/lorie/.../MainActivity.java:195-206).
        ...
        // receiver (necesario en termux-x11 original para arrancar el servidor X vía shell,
        // UID distinto — CmdEntryPoint.java documenta que Kairos ya no usa ese modo, el
        // servidor corre siempre dentro del propio proceso ":xserver") permitía que CUALQUIER
        // app del dispositivo mandara ACTION_STOP/ACTION_START a este receiver. ContextCompat
        // resuelve ambos: usa el flag real en API 33+ (no exportado) y cae al registro de 2
        // argumentos en versiones previas (donde Android no puede distinguir exported/no
        // exported para receivers dinámicos — limitación real de esas versiones de la
        // plataforma, no algo que el código de la app pueda cerrar del todo sin agregar un
        // permiso propio).
        ContextCompat.registerReceiver(this, receiver, new IntentFilter(ACTION_START) {{
...
//        });
     ...
     *   el visor. Es la misma semántica que el botón "Cerrar servidor X11" de la
     *   pantalla X11 de Kairos (X11Fragment).
     *
     * Nota: el listener de teclado (mLorieKeyListener) sigue consumiendo el back como
     * evento del teclado para mandarlo al X11 cuando hay teclado físico; este override
     * cubre el gesto/botón back del sistema (onBackPressed), que es el que usaba el
     * usuario y que antes no hacía nada visible (el key listener lo tragaba).
     */
    ...
    private void showExitDialog() {
        new AlertDialog.Builder(this)
            .setTitle("Salir de X11")
            .setItems(new CharSequence[]{"Minimizar", "Cerrar servidor X11"}, (d, which) -> {
                if (which == 0) {
                    moveTaskToBack(true);
                } else {
                    stopX11Server();
    ...
    /**
     * Kairos: detiene el servidor X11 y cierra el visor. Como X11Service vive en el
     * módulo :app (com.termux.app.X11Service, proceso :xserver) y este visor está en
     * :x11-server (que no puede depender de :app — dependencia circular), se para el
     * servicio por nombre de componente explícito en vez de referenciar la clase.
     * Mismo contrato que X11Fragment.stopX11(): broadcast ACTION_STOP → finishAffinity
     * + stop del servicio :xserver.
     */
    private void stopX11Server() {
        try {
            sendBroadcast(new Intent(ACTION_STOP).setPackage(getPackageName()));
        } catch (Exception ignored) {
            // finishAffinity de todas formas abajo
        }
        try {
            stopService(new Intent().setComponent(
                new ComponentName(getPackageName(), "com.termux.app.X11Service")));
        } catch (Exception e) {
            Log.e("MainActivity", "No se pudo detener el servicio X11", e);
        }
        finishAffinity();
    }
